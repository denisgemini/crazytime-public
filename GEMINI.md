# INSTRUCCIONES CRÍTICAS PARA EL AGENTE (GEMINI) - CrazyTime v2.5
...
1. **JAMÁS** utilices la vista `tiros_ordenados` o el campo `pseudo_id` para lógica de tracking o cálculos. Usar siempre `id` real.
...
1. **Fuente de Verdad:** La tabla `tiros` (ID real) y los JSONs en `data/distances/`.
2. **Lógica de Ventana:**
...
3. **Reporte Diario:** Debe ser festivo, estratégico y centrado exclusivamente en la rentabilidad de las ventanas.
4. **Intervalo:** Recolección cada 5 minutos (300s).

## 🎯 OBJETIVO ACTUAL
Diseñar e implementar la "Caché de Día" para obtener datos en tiempo real (1 min) sin ensuciar la base de datos consolidada.